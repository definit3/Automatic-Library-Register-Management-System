		...
		if filename.endswith(".jpg"):
			...
			for face  in res:
				if not face['candidates']:
					...
					flag=1
					for i in range(1,x):
						if ws.cell(row=i,column=2).value==row[1]:
							if ws.cell(row=i,column=5).value is None:
								print 'Please take the exit from prev entry first'
								flag=0
								break
					
					if flag:
						ws.cell(row=x,column=1,value=row[2])
						ws.cell(row=x,column=2,value=row[1])
						date=datetime.date.today()
						time=datetime.datetime.now().time()
						ws.cell(row=x,column=4,value=time)
						ws.cell(row=x,column=3,value=date)
						wb.save(filename='workbook.xlsx')
					
	 print len(dets)
	 for i in range(1,len(dets)+1):
		print 'ok'
	 	os.remove('./detected_face/face'+str(i)+'.jpg')
